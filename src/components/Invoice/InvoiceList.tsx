...
const TabContainer = styled.div`
  ...
  margin-left: auto;
  margin-right: auto;
  border: 1px solid ${({ theme }) => theme.mode === 'dark' ? '#334155' : 'transparent'};
`;
...
const Tab = styled.button<{ active: boolean }>`
  ...
  &:hover {
    background: ${({ theme, active }) => {
      ...
      return theme.mode === 'dark' ? 'rgba(96, 165, 250, 0.1)' : 'rgba(102, 126, 234, 0.05)';
    }};
...
const StatNumber = styled.div`
  font-size: 32px;
  font-weight: 800;
  color: ${({ theme }) => theme.mode === 'dark' ? '#60a5fa' : '#667eea'};
  margin-bottom: 8px;
`;
...
const LoadingSpinner = styled.div`
  ...
  width: 40px;
  height: 40px;
  border: 4px solid ${({ theme }) => theme.mode === 'dark' ? '#334155' : '#e2e8f0'};
  border-radius: 50%;
  border-top-color: ${({ theme }) => theme.mode === 'dark' ? '#60a5fa' : '#667eea'};
